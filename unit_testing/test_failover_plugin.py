...
class TestFailoverPlugin(TestCase):
    ...
    def test_init_host_provider_failover_disabled(self):
        plugin_service_mock: PluginService = MagicMock()
        host_list_provider_service_mock: HostListProviderService = MagicMock()
    ...
    def test_init_host_provider_with_static_host_provider(self):
        plugin_service_mock: PluginService = MagicMock()
        host_list_provider_service_mock: HostListProviderService = MagicMock()
        init_host_provider_func_mock: Callable = MagicMock()
    ...
    def test_failover_reader(self):
        ...
        with mock.patch.object(FailoverPlugin, "_failover_reader") as failover_reader_mock:
            host_mock: HostInfo = MagicMock()
            self.assertRaises(FailoverSuccessError, plugin._failover, host_mock)
            failover_reader_mock.assert_called_once_with(host_mock)

    def test_failover_writer(self):
        plugin_service_mock: PluginService = MagicMock()
    ...
    def test_failover_reader_with_valid_failed_host(self):
        plugin_service_mock: PluginService = MagicMock()
        conn_mock: Connection = MagicMock()
        reader_failover_handler_mock: ReaderFailoverHandler = MagicMock()
    ...
    def test_failover_reader_with_no_failed_host(self):
        plugin_service_mock: PluginService = MagicMock()
        reader_failover_handler_mock: ReaderFailoverHandler = MagicMock()
        host: HostInfo = HostInfo("host")
        host.availability = HostAvailability.AVAILABLE
        ...
        with mock.patch.object(reader_failover_handler_mock, "failover") as failover_reader_mock:
            ...
            self.assertRaises(Error, plugin._failover_reader, None)
            failover_reader_mock.assert_called_with(hosts, None)

    def test_failover_writer_failed_failover_raises_error(self):
        plugin_service_mock: PluginService = MagicMock()
        ...
        with mock.patch.object(writer_failover_handler_mock, "failover") as failover_writer_mock:
            ...
            self.assertRaises(Error, plugin._failover_writer)
            failover_writer_mock.assert_called_with(hosts)

    def test_failover_writer_failed_failover_with_no_result(self):
        plugin_service_mock: PluginService = MagicMock()
        writer_failover_handler_mock: WriterFailoverHandler = MagicMock()
        host: HostInfo = HostInfo("host")
        host._aliases = ["alias1", "alias2"]
    ...
    def test_failover_writer_success(self):
        plugin_service_mock: PluginService = MagicMock()
        writer_failover_handler_mock: WriterFailoverHandler = MagicMock()
        host: HostInfo = HostInfo("host")
        host._aliases = ["alias1", "alias2"]
    ...
    def test_invalid_current_connection_with_no_connection(self):
        plugin_service_mock: PluginService = MagicMock()
    ...
    def test_invalid_current_connection_in_transaction(self):
        conn_mock: Connection = MagicMock()
        plugin_service_mock: PluginService = MagicMock()
        type(plugin_service_mock).is_in_transaction = PropertyMock(return_value=True)
        type(plugin_service_mock).current_connection = PropertyMock(return_value=conn_mock)
        ...
        try:
            ...
            self.fail("_invalidate_current_connection() raised unexpected error")

    def test_invalidate_current_connection_not_in_transaction(self):
        conn_mock: Connection = MagicMock()
        plugin_service_mock: PluginService = MagicMock()
    ...
    def test_execute(self):
        ...
        def mock_sql_method():
            TestFailoverPlugin.mock_method_called = True
