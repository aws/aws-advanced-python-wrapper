...
The GDB Read/Write Splitting Plugin can be used against Aurora and RDS clusters. However, since these cluster types are single-region, configuring a home region adds little value.
...
| `gdb_rw_restrict_writer_to_home_region`| Boolean |                                                                     No                                                                     | If set to `True`, prevents following or connecting to a writer node outside the defined home region. An exception will be raised when such a connection to a writer outside the home region is requested.                                                                                                                                                                                          | `True`                                                                                                                           |
