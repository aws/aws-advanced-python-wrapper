...
| `weighted_random`                   | The weighted random strategy will be chosen randomly from the available database instances. A slight addition to the random strategy is the weighted random strategy, where more connections will be passed to reader instances based on user specified connection properties.                                                                                                                                                                                                                                                                                               | See the rows `weighted_random_host_weight_pairs` and `weighted_random_default_weight` for configuration parameters |
| `weighted_random_host_weight_pairs` | This parameter value must be a `string` type comma separated list of database host-weight pairs in the format `<host>:<weight>`. The host represents the database instance name, and the weight represents the probability of a connection being directed to a particular host. For example, the value `instance-1:1,instance-2:4` means that connections to `instance-2` will be four times as likely to direct connections to `instance-2` compared to `instance-1`. <br><br> **Note:** The `<weight>` value in the string must be an integer greater than or equal to 1.  | `""` (empty string)                                                                                                |
