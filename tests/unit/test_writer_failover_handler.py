...
@pytest.fixture
def setup(self, writer: HostInfo, new_writer_host: HostInfo, reader_a: HostInfo, reader_b: HostInfo):
    writer.add_alias("writer-host")
    new_writer_host.add_alias("writer-host")
    reader_a.add_alias("writer-host")
    reader_b.add_alias("writer-host")


def test_reconnect_to_writer_task_b_reader_exception(connection_mock, plugin_service_mock, reader_failover_mock, default_properties, writer, reader_a,
                                                     reader_b, topology):
...
def test_reconnect_to_writer_slow_reader_a(plugin_service_mock, reader_failover_mock, writer_connection_mock, new_writer_connection_mock,
    ...
    plugin_service_mock.hosts.side_effect = [current_topology, new_topology]
