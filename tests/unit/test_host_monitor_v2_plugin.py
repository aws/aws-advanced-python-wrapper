...
def test_set_node_unhealthy(monitoring_context):
    assert monitoring_context._node_unhealthy.get() is False
    monitoring_context.set_node_unhealthy()
    assert monitoring_context._node_unhealthy.get() is True


def test_should_abort_when_healthy(monitoring_context, mock_conn):
    assert monitoring_context.should_abort() is False


def test_should_abort_when_unhealthy(monitoring_context, mock_conn):
    monitoring_context.set_node_unhealthy()
    assert monitoring_context.should_abort() is True


def test_should_abort_when_inactive(monitoring_context, mock_conn):
    monitoring_context.set_node_unhealthy()
...
@patch.object(HostMonitorV2, '_execute_conn_check')
def test_is_host_available_operational_error(mock_execute, host_monitor, mock_conn):
    mock_execute.side_effect = psycopg.OperationalError()
    assert host_monitor._is_host_available(mock_conn, 1.0) is False


def test_execute_conn_check(host_monitor, mock_conn, mock_plugin_service):
...
def test_update_node_health_status_multiple_failures(host_monitor):
    ...
    host_monitor._invalid_node_start_time_ns = 10**9

    host_monitor._update_node_health_status(False, 10**9, 11 * 10**9)
...
def test_update_node_health_status_recovery(host_monitor):
    ...
    host_monitor._update_node_health_status(True, 2 * 10**9, 2.1 * 10**9)

    assert host_monitor._failure_count == 0
    assert host_monitor._invalid_node_start_time_ns == 0
    assert host_monitor._is_unhealthy is False


def test_abort_connection_success(host_monitor, mock_conn):
    host_monitor.abort_connection(mock_conn)
    mock_conn.close.assert_called_once()


@patch('aws_advanced_python_wrapper.host_monitoring_v2_plugin.logger')
def test_abort_connection_failure(mock_logger, host_monitor, mock_conn):
    mock_conn.close.side_effect = AwsWrapperError("Error")
    host_monitor.abort_connection(mock_conn)
