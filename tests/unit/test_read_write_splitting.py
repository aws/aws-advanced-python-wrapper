...
from aws_advanced_python_wrapper.errors import AwsWrapperError, FailoverSuccessError, ReadWriteSplittingError
...
from aws_advanced_python_wrapper.simple_read_write_splitting_plugin import SimpleReadWriteSplittingPlugin
...
@pytest.fixture
def plugin_instance(plugin_config, plugin_service_mock, plugin_props, host_list_provider_service_mock):
...
def test_set_read_only_true(plugin_instance, plugin_service_mock, plugin_config, reader_conn_mock):
    plugin_name, _ = plugin_config
    plugin_service_mock.current_connection = writer_conn_mock
...
def test_set_read_only_false(plugin_instance, plugin_service_mock, plugin_config, reader_conn_mock, writer_conn_mock):
    plugin_name, _ = plugin_config
...
def test_set_read_only_true_already_on_reader(plugin_instance, plugin_service_mock, plugin_config, reader_conn_mock):
    plugin_name, _ = plugin_config
    plugin_service_mock.current_connection = reader_conn_mock
...
def test_set_read_only_false_already_on_writer(plugin_instance, plugin_service_mock, plugin_config, writer_conn_mock):
    plugin_name, _ = plugin_config
    plugin_service_mock.current_connection = writer_conn_mock
...
def test_set_read_only_false_in_transaction(plugin_instance, plugin_service_mock, reader_conn_mock):
    plugin_service_mock.current_connection = reader_conn_mock
...
def test_set_read_only_true_in_transaction_already_on_reader(plugin_instance, plugin_service_mock, reader_conn_mock):
    plugin_service_mock.current_connection = reader_conn_mock
    plugin_service_mock.is_in_transaction = True
    plugin_service_mock.current_host_info = simple_reader_host
...
def test_set_read_only_on_closed_connection(plugin_instance, plugin_service_mock, closed_writer_conn_mock):
...
def test_notify_connection_change(plugin_instance, plugin_service_mock, writer_conn_mock):
...
def test_connect_non_initial_connection(plugin_instance, plugin_config, connect_func_mock, writer_conn_mock, mocker):
    plugin_name, _ = plugin_config
    ...
    if plugin_name == "read_write_splitting":
        ...
        conn = plugin_instance.connect(
            mocker.MagicMock(), mocker.MagicMock(), writer_host, Properties(), False, connect_func_mock)
        
        assert conn == writer_conn_mock
        connect_func_mock.assert_called()
    else:
        result = plugin_instance.connect(None, None, simple_writer_host, Properties(), False, connect_func_mock)
        
        assert result == writer_conn_mock
        connect_func_mock.assert_called_once()

def test_close_pooled_reader_connection_after_set_read_only(plugin_props, plugin_service_mock, plugin_config, mocker, reader_conn_mock, writer_conn_mock):
    plugin_name, plugin_class = plugin_config
    def connect_side_effect(host: HostInfo, props, plugin):
        if host in [reader_host1, reader_host2, reader_host3] or host.host == READ_ENDPOINT:
...
def test_close_pooled_writer_connection_after_set_read_only(plugin_service_mock, plugin_config, plugin_props, mocker, reader_conn_mock, writer_conn_mock):
    plugin_name, plugin_class = plugin_config
    def connect_side_effect(host: HostInfo, props, plugin):
        if host in [reader_host1, reader_host2, reader_host3] or host.host == READ_ENDPOINT:
            return reader_conn_mock
...
def test_set_read_only_false_writer_connection_fails(plugin_instance, plugin_service_mock, reader_conn_mock):    
    def connect_side_effect(host_info: HostInfo, props: Properties, plugin):
        if (host_info == writer_host or host_info.host == WRITE_ENDPOINT) and props == Properties():
...
def test_set_read_only_true_reader_connection_failed(plugin_instance, plugin_service_mock, writer_conn_mock):
    def connect_side_effect(host_info: HostInfo, props: Properties, plugin):
...
def test_execute_failover_to_new_writer(plugin_instance, plugin_service_mock, writer_conn_mock, new_writer_conn_mock):
    def execute_func():
...
def test_connect_incorrect_host_role(plugin_instance, plugin_service_mock, plugin_config, mocker, connect_func_mock, reader_conn_mock, host_list_provider_service_mock):    
    plugin_name, _ = plugin_config
    ...
    conn = plugin_instance.connect(
        mocker.MagicMock(), mocker.MagicMock(), writer_host, Properties(), True, connect_func_mock)
...
def test_connect_error_updating_host(plugin_service_mock, plugin_config, plugin_instance, host_list_provider_service_mock, connect_func_mock, mocker):    
    plugin_name, _ = plugin_config
    if plugin_name != "read_write_splitting":
       pytest.skip("Test only applies to Read/Write Splitting Plugin")
...
def test_constructor_invalid_initial_connection_type(plugin_service_mock, plugin_config):  
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
    ...
    props[WrapperProperties.SRW_WRITE_ENDPOINT.name] = WRITE_ENDPOINT
    props[WrapperProperties.SRW_READ_ENDPOINT.name] = READ_ENDPOINT
    props[WrapperProperties.SRW_VERIFY_INITIAL_CONNECTION_TYPE.name] = "other" # "writer", "reader" are the only valid options

    with pytest.raises(ValueError):
            SimpleReadWriteSplittingPlugin(plugin_service_mock, props)

def test_connect_verification_disabled(plugin_service_mock, plugin_config, connect_func_mock, writer_conn_mock):  
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
    ...
    result = plugin.connect(None, None, simple_writer_host, props, True, connect_func_mock)

    assert result == writer_conn_mock
    connect_func_mock.assert_called_once()

def test_connect_writer_cluster_endpoint(plugin_config, plugin_instance, plugin_service_mock, plugin_props, connect_func_mock, writer_conn_mock):
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
    ...
    result = plugin_instance.connect(None, None, writer_cluster_host, plugin_props, True, connect_func_mock)

    assert result == writer_conn_mock
    connect_func_mock.assert_called_once()
    assert plugin_service_mock.get_host_role.call_count == 1

def test_connect_reader_cluster_endpoint(plugin_config, plugin_instance, plugin_props, plugin_service_mock, connect_func_mock, reader_conn_mock):
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
    ...
    result = plugin_instance.connect(None, None, reader_cluster_host, plugin_props, True, connect_func_mock)

    assert result == reader_conn_mock
    connect_func_mock.assert_called_once()
    assert plugin_service_mock.get_host_role.call_count == 1

def test_connect_verification_fails_fallback(plugin_config, plugin_service_mock, connect_func_mock, writer_conn_mock, host_list_provider_service_mock):
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
    ...
    props[WrapperProperties.SRW_CONNECT_RETRY_INTERVAL_MS.name] = "6" # Interval > timeout ensures only one call before fallsback
...
def test_connect_non_rds_cluster_endpoint(plugin_config, plugin_instance, plugin_service_mock, plugin_props, connect_func_mock, writer_conn_mock):
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
    ...
    result = plugin_instance.connect(None, None, custom_host, plugin_props, True, connect_func_mock)

    assert result == writer_conn_mock
    connect_func_mock.assert_called_once()
    assert plugin_service_mock.get_host_role.call_count == 0

def test_connect_non_rds_cluster_endpoint_with_verification(plugin_config, plugin_service_mock, plugin_props, connect_func_mock, writer_conn_mock, mocker):
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
    ...
    props[WrapperProperties.SRW_WRITE_ENDPOINT.name] = WRITE_ENDPOINT
    props[WrapperProperties.SRW_READ_ENDPOINT.name] = READ_ENDPOINT
    props[WrapperProperties.SRW_VERIFY_INITIAL_CONNECTION_TYPE.name] = "writer"  # Forces verification to a writer

    connect_func_mock.return_value = writer_conn_mock
    plugin_service_mock.get_host_role = mocker.MagicMock(side_effect=[HostRole.READER, HostRole.WRITER])

    plugin = SimpleReadWriteSplittingPlugin(plugin_service_mock, props)
    plugin._connection_handler._host_list_provider_service = host_list_provider_service_mock

    result = plugin.connect(None, None, custom_host, plugin_props, True, connect_func_mock)

    assert result == writer_conn_mock
    assert connect_func_mock.call_count == 2
    assert plugin_service_mock.get_host_role.call_count == 2

def test_wrong_role_connection_writer_endpoint_to_reader(plugin_service_mock, reader_conn_mock, plugin_config, plugin_instance):
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
...
def test_get_verified_connection_wrong_role_retry_reader(plugin_config, plugin_instance, plugin_service_mock, reader_conn_mock, writer_conn_mock):        
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
...
def test_get_verified_connection_sql_exception_retry(plugin_config, plugin_instance, plugin_service_mock, reader_conn_mock, writer_conn_mock):   
    plugin_name, _ = plugin_config
    if plugin_name != "srw":
       pytest.skip("Test only applies to Simple Read/Write Splitting Plugin")
