...
class LimitlessConnectionPlugin(Plugin):
    ...
    def _connect(
        ...
        limitless_router_service = LimitlessRouterService(
            ...
            self.initialize_limitless_router_monitor,
            LimitlessQueryHelper(self._plugin_service)
        )

        if is_initial_connection:
            limitless_router_service.start_monitoring(host_info, properties, 7500)
    ...
    def initialize_limitless_router_monitor(self, plugin_service: PluginService,
                                            host_info: HostInfo,
                                            limitless_router_cache: SlidingExpirationCacheWithCleanupThread,
                                            limitless_router_cache_key: str,
                                            props: Properties,
                                            interval_ms: int):
        return LimitlessRouterMonitor(plugin_service,
                                      host_info,
                                      limitless_router_cache,
                                      limitless_router_cache_key,
                                      props,
                                      interval_ms)
...
class LimitlessRouterMonitor:
    ...
    def run(self):
        ...
        try:
            while not self.is_stopped:
                ...
                if self._monitoring_conn is not None:
                    ...
                    self._limitless_router_cache.compute_if_absent(self._limitless_router_cache_key,
                                                                   ...
                                                                   WrapperProperties.MONITOR_DISPOSAL_TIME_MS.get(self._properties))
            ...
            logger.debug("LimitlessRouterMonitor.ExceptionDuringMonitoringStop", self.host_info.host)
...
class LimitlessQueryHelper:
    ...
    _DEFAULT_QUERY_TIMEOUT_MS: int = 5000

    def __init__(self, plugin_service: PluginService):
        self._plugin_service = plugin_service

    def query_for_limitless_routers(self, connection: Connection, host_post_to_map: int) -> List[HostInfo]:
        ...
        with closing(connection.cursor()) as cursor:
            self._plugin_service.driver_dialect.execute("Cursor.execute",
                                                        ...
                                                        query,
                                                        exec_timeout=LimitlessQueryHelper._DEFAULT_QUERY_TIMEOUT_MS)
            return self._map_result_set_to_host_info_list(cursor.fetchall(), host_post_to_map)

    def _map_result_set_to_host_info_list(self, result_set: List[Tuple[Any, Any]], host_post_to_map: int) -> List[HostInfo]:
        print(result_set)
        list_of_host_infos: List[HostInfo] = []
        for result in result_set:
            list_of_host_infos.append(self._create_host_info(result, host_post_to_map))
        return list_of_host_infos

    def _create_host_info(self, result: Tuple[Any, Any], host_post_to_map: int) -> HostInfo:
        ...
        return HostInfo(host_name, host_post_to_map, weight=weight, host_id=host_name)
...
class LimitlessConnectionContext:
    ...
    def is_any_routers_available(self):
...
class LimitlessRouterService:
    ...
    _limitless_router_cache: ClassVar[SlidingExpirationCacheWithCleanupThread[str, List[HostInfo]]] = \
        SlidingExpirationCacheWithCleanupThread(_CACHE_CLEANUP_NS,
                                                should_dispose_func=lambda x: True,
                                                item_disposal_func=lambda x: {})
    ...
    def __init__(self, plugin_service: PluginService, create_limitless_router_monitor: Callable, query_helper: LimitlessQueryHelper):
        self._plugin_service = plugin_service
        self._create_limitless_router_monitor = create_limitless_router_monitor
        self._query_helper = query_helper

        self._force_get_limitless_routers_lock_map: ConcurrentDict[str, RLock] = ConcurrentDict()
        self._rds_utils = RdsUtils()

    def establish_connection(self, context: LimitlessConnectionContext) -> None:
        context.set_limitless_routers(self._get_limitless_routers(context.get_host_info().host_id, context.get_props()))
        ...
        try:
            selected_host_info = self._plugin_service.get_host_info_from_input_by_strategy(
                context.get_limitless_routers(), HostRole.WRITER, "weighted_random")
        ...
        try:
            context.set_connection(self._plugin_service.connect(selected_host_info, context.get_props()))
        except Exception as e:
            if self._is_login_exception(e):
                raise e
    ...
    def _get_limitless_routers(self, cluster_id: str, props: Properties) -> List[HostInfo]:
        ...
        cache_expiration_nano: int = WrapperProperties.MONITOR_DISPOSAL_TIME_MS.get_int(props) * 1_000_000
    ...
    def _retry_connection_with_least_loaded_routers(self, context: LimitlessConnectionContext) -> None:
        retry_count = 0
        max_retries = WrapperProperties.MAX_RETRIES.get_int(context.get_props())
        while retry_count < max_retries:
            retry_count += 1
            if context.get_limitless_routers() is None or len(context.get_limitless_routers()) == 0 or not context.is_any_routers_available():
                self._synchronously_get_limitless_routers_with_retry(context)

                if (context.get_limitless_routers() is None
                        or len(context.get_limitless_routers()) == 0
                        or not context.is_any_routers_available()):
    ...
    def _synchronously_get_limitless_routers_with_retry(self, context: LimitlessConnectionContext) -> None:
        ...
        retry_count = -1
        max_retries = WrapperProperties.MAX_RETRIES.get_int(context.get_props())
        retry_interval_ms = WrapperProperties.GET_ROUTER_RETRY_INTERVAL_MILLIS.get_float(context.get_props())
    ...
    def _synchronously_get_limitless_routers(self, context: LimitlessConnectionContext) -> None:
        cache_expiration_nano: int = WrapperProperties.MONITOR_DISPOSAL_TIME_MS.get_int(context.get_props()) * 1_000_000

        lock = self._force_get_limitless_routers_lock_map.compute_if_absent(
    ...
    def clear_cache(self) -> None:
        self._force_get_limitless_routers_lock_map.clear()
