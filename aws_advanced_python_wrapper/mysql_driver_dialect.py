    ...
    def is_closed(self, conn: Connection) -> bool:
        if isinstance(conn, CMySQLConnection) or isinstance(conn, MySQLConnection):
            ...
            if self.can_execute_query(conn):
                socket_timeout = WrapperProperties.SOCKET_TIMEOUT_SEC.get(self._props)
                timeout_sec = socket_timeout if socket_timeout is not None else MySQLDriverDialect.IS_CLOSED_TIMEOUT_SEC
