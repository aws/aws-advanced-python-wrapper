...
class MonitoringContext:
    ...
    def is_active(self) -> bool:
        connection_weak_ref = self._connection_to_abort.get()
        if connection_weak_ref is not None:
            connection = connection_weak_ref()
...
class HostMonitorV2:
    ...
    def _new_context_run(self) -> None:
            ...
            while not self.is_stopped:
                ...
                for key in keys:
                    ...
                    while not queue.empty():
                        ...
                        if context_weak_ref is not None and context_weak_ref() is not None:
                            active = context_weak_ref().is_active()
                            if active:
        ...
        self._monitor_new_context_thread.join(5)
        ...
        try:
            while not self.is_stopped:
                if self._active_contexts.empty() and not self._is_unhealthy:
                    sleep(HostMonitorV2._THREAD_SLEEP_NANO / 1_000_000_000)
                    continue
            ...
            if self._monitoring_connection is not None:
                try:
                    self._driver_dialect.abort_connection(self._monitoring_connection)
                except AwsWrapperError as ex:
                    logger.debug(ex)
                    pass

            logger.debug("HostMonitorV2.StopMonitoringThread", self._host_info.host)
            self._monitor_run_thread.join(5)
    ...
    def check_connection_status(self) -> bool:
        ...
        try:
            if self._monitoring_connection is None or self._driver_dialect.is_closed(self._monitoring_connection):
                ...
                return True
            valid_timeout = ((self._failure_detection_interval_ns - self._THREAD_SLEEP_NANO) / 2) / 1_000_000_000
    ...
    def abort_connection(self, connection: Connection) -> None:
        try:
            if self._driver_dialect.is_closed(connection):
                return
