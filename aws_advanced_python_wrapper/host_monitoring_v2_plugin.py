...
class HostMonitoringV2Plugin(Plugin, CanReleaseResources):
    ...
    def execute(self, target: object, method_name: str, execute_func: Callable, *args: Any, **kwargs: Any) -> Any:
        ...
        failure_detection_time_ms = WrapperProperties.FAILURE_DETECTION_TIME_MS.get_int(self._properties)
        failure_detection_interval_ms = WrapperProperties.FAILURE_DETECTION_INTERVAL_MS.get_int(self._properties)
        failure_detection_count = WrapperProperties.FAILURE_DETECTION_COUNT.get_int(self._properties)
    ...
    def _get_monitoring_host_info(self) -> HostInfo:
        if self._monitoring_host_info is None:
            ...
            try:
                if rds_url_type.is_rds_cluster:
...
class HostMonitorV2:
    ...
    def __init__(
        ...
        self._invalid_node_start_time_ns: int = 0
    ...
    def _new_context_run(self) -> None:
        ...
        try:
            while not self.is_stopped:
                ...
                for key in keys:
                    queue: Queue | None = self._new_contexts.get(key)
                    if queue is not None:
                        if key < current_time_ns:
                            processed_keys.append(key)
                            while not queue.empty():
                                context_weak_ref = queue.get()
                                if context_weak_ref is not None and context_weak_ref() is not None and context_weak_ref().is_active():
            ...
            if self._monitoring_connection is None or self._driver_dialect.is_closed(self._monitoring_connection):
                ...
                for property_key in monitoring_properties.keys():
                    if property_key.startswith(self._MONITORING_PROPERTY_PREFIX):
                        monitoring_properties[property_key[len(self._MONITORING_PROPERTY_PREFIX):]] = \
    ...
    def _update_node_health_status(
        ...
        if not connection_valid:
            ...
            if self._invalid_node_start_time_ns == 0:
                self._invalid_node_start_time_ns = int(status_check_start_ns)

            invalid_node_duration_ns = status_check_end_ns - self._invalid_node_start_time_ns
            max_invalid_node_duration_ns = self._failure_detection_interval_ns * max(0,
                                                                                     self._failure_detection_count - 1)
